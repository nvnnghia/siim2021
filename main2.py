...
if 'eff' in cfg.model_architecture:
    from model_5_1 import EfficientNet
else:
    from model_6_1 import SIIMModel
...
def get_model2(cfg):
    model = EfficientNet(model_name=cfg.model_architecture)
    return model
...
def get_dataloader(cfg, fold_id):
    ...
    if cfg.mode in ['test']:
        df = pd.read_csv('../../../../../image_siim/meta_test_final.csv')
...
def train_func(model, train_loader, scheduler, device, epoch):
    ...
    for batch_idx, batch_data in enumerate(bar):
        if cfg.use_seg:
            ...
            images, targets, oof_targets, targets1,masks = batch_data
        ...
        if cfg.model in ['model_2_1']:
            ...
            prediction, prediction1, out_mask = predictions
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(bar):
            if cfg.use_seg:
                ...
                images, targets, image_ids, targets1,masks = batch_data
            ...
            if cfg.model in ['model_2_1']:
                logits, prediction1,out_mask = predictions
            elif cfg.model in ['model_4_1']:
...
def test_func(model, valid_loader):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(bar):
            ...
            if cfg.model in ['model_2_1']:
                logits, prediction1,_ = predictions
            elif cfg.model in ['model_4_1']:
                logits, prediction1, seg_out = predictions
            elif cfg.model in ['model_4']:
                logits, seg_out = predictions
    ...
    for cc, fold_id in enumerate(cfg.folds):
        ...
        if cfg.mode == 'train':
            ...
            if cfg.weight_file:
                ...
                if '.pth' or '.bin' in cfg.weight_file:
                    weight_file = cfg.weight_file
                else:
                    weight_file = cfg.weight_file + f'best_map_fold{fold_id}_st0.pth'
                state_dict = torch.load(weight_file, map_location=device)['model_state_dict']  # load checkpoint
                if 'eff' in cfg.model_architecture:
                    del state_dict['fc_b4.weight'],state_dict['fc_b4.bias'],state_dict['fc_b5.weight'],state_dict['fc_b5.bias'],\
                    state_dict['fc.weight'],state_dict['fc.bias']
                else:
                    del state_dict['fc_b5.weight'],state_dict['fc_b5.bias'],\
                     state_dict['fc.weight'],state_dict['fc.bias']
        ...
        if cfg.mode in['train', 'val']:
            ...
            model = get_model(cfg).to(device)
            # chpt_path = f'{cfg.out_dir}/last_checkpoint_fold{fold_id}_st{cfg.stage}.pth'
            chpt_path = f'{cfg.out_dir}/best_map_fold{fold_id}_st{3}.pth'
            # chpt_path = f'{cfg.out_dir}/best_loss_fold{fold_id}_st{cfg.stage}.pth'
            checkpoint = torch.load(chpt_path, map_location="cpu")
            if cfg.use_seg:
                model.model.load_state_dict(checkpoint)
            ...
            if cc == (len(cfg.folds)-1):
                ...
                val_df.to_csv(f'{cfg.out_dir}/{cfg.name}_test_st{3}.csv', index=False)

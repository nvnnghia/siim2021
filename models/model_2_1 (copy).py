...
class SIIMModel(nn.Module):
    def __init__(self, model_name='resnet200d', out_dim=4, pretrained=False, dropout=0.5,
        ...
        if 'resne' in model_name: #resnet
            ...
            self.fc_b5 = nn.Linear(self.model.stages[-2][-1].conv3.out_channels+64, out_dim)
        ...
        self.fc1_b5 = nn.Linear(5, 64)
        self.fc1 = nn.Linear(5, 64)

        self.fc = nn.Linear(n_features+64, out_dim)
    ...
    def forward(self, ipt):
        ...
        a = torch.tensor([0.402, 0.70, 0.275, 0.112, 1]).cuda()
        a = torch.stack([a for _ in range(bs)])

        x2, x3, x4 = self._features(ipt)
        # print(x0.shape, x1.shape, x2.shape, x3.shape, x4.shape)

        feat5 = torch.flatten(self.global_pool(self.bottleneck_b5(x3)), 1)
        fc_feat5 = self.fc1_b5(a)

        # print(fc_feat5.shape, feat5.shape)

        feat5 = torch.cat([feat5, fc_feat5], 1)

        # print(feat5.shape)

        b5_logits = self.fc_b5(feat5)

        pooled_features = self.pooling(x4).view(bs, -1)

        feat6 = self.dropout(pooled_features)
        fc_feat6 = self.fc1(a)
        feat6 = torch.cat([feat6, fc_feat6], 1)

        cls_logit = self.fc(feat6)

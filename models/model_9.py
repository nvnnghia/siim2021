...
class SIIMModel(nn.Module):
    def __init__(self, model_name='resnet200d', out_dim=4, pretrained=False, dropout=0.5,
        ...
        if 'resne' in model_name: #resnet
            ...
            n_features = self.model.final_conv.out_channels + self.model.stages[-2][-1].conv3.out_channels
    ...
    def forward(self, ipt):
        ...
        # torch.Size([32, 128, 96, 96]) torch.Size([32, 256, 96, 96]) torch.Size([32, 512, 48, 48]) torch.Size([32, 1536, 24, 24]) torch.Size([32, 3072, 12, 12])

        b5_features = self.global_pool(self.bottleneck_b5(x3)).view(bs, -1)
        b4_features = self.global_pool(self.bottleneck_b5(x2)).view(bs, -1)
        b3_features = self.global_pool(self.bottleneck_b5(x1)).view(bs, -1)
        b2_features = self.global_pool(self.bottleneck_b5(x0)).view(bs, -1)

        pooled_features = self.pooling(x4).view(bs, -1)
        features = torch.cat([b2_features, b3_features, b4_features, b5_features, pooled_features], 1)

        # print(b5_features.shape, pooled_features.shape)
        # print(features.shape)

        cls_logit = self.fc(self.dropout(features))

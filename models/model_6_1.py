...
class SIIMModel(nn.Module):
    def __init__(self, model_name='resnet200d', out_dim=4, pretrained=False, dropout=0.5,
        ...
        if 'resne' in model_name: #resnet
            ...
            # self.bottleneck_b4 = nn.Identity()
            # self.fc_b4 = nn.Linear(self.model.stages[-3][-1].conv3.out_channels, out_dim)
    ...
    def forward(self, ipt):
        ...
        b4_logits = self.fc_b4(torch.flatten(self.global_pool(self.bottleneck_b4(x2)), 1))
        ...
        return cls_logit, b5_logits, self.mask(x3)

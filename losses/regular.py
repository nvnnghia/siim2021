...
def ce(weight=None, reduction='mean'):
    print('[ √ ] Using CE loss, weight is {}'.format(weight))
    ce = CrossEntropyLoss(weight=weight, reduction=reduction)
...
def weighted_bce(reduction='mean', weight=[0]):
    # print('[ √ ] Using loss MSE')
    weight = torch.tensor(weight).cuda()
    def _bce_loss(output, truth, w=True):
        bce = BCEWithLogitsLoss(reduction=reduction)
        raw = bce(output, truth)
        if w:
            return raw * weight
        else:
            return raw
...
def label_smooth_ce(eps=0.1, reduction='mean', weight=None):
    def _ce_loss(output, gt):
        ce = LabelSmoothingCrossEntropy(eps=eps, reduction=reduction, weight=weight)

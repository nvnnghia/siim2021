...
def detect1Image(im0, imgsz, model, device, conf_thres, iou_thres):
    ...
    (pred, _), seg_out = model(img, augment=False) 
    ...
    return np.array(boxes), np.array(scores), np.array(classes), seg_out.detach().cpu().numpy()
        ...
        for img_path, lb_path in zip(image_list, label_list):
            ...
            if is_TTA:
                ...
                for model, imgz, rot in zip(models, imgsz, rots):
                    ...
                    for _ in range(rot):
                        imgRotate = cv2.rotate(imgRotate, cv2.ROTATE_90_CLOCKWISE)
                    boxes, scores, classes, seg_out = detect1Image(imgRotate, imgz, model, device, conf_thres, iou_thres)
                    if boxes.shape[0]>0:
                        for _ in range(4-rot):
                            boxes = rotBoxes90(boxes, im_w, im_h)
                        list_boxes.append(boxes)
...
if __name__ =="__main__":
    ...
    weights = ['runs/vds_cf5_s/exp27/weights/best.pt'] 
    outname = 'runs/x_exp_last.txt'
    sizes = [640]
    rots = [0]
    num_classes = 4
    ...
    image_list, label_list = get_data('/home/pintel/nvnn/dataset/vds_vid/val.txt', key=None)

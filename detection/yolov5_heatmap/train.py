...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
            ...
            print('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), model))  # report
    ...
    if hasattr(model, 'model'):
        ...
        nl = model.model.model[-1].nl  # number of detection layers (used for scaling hyp['obj'])
    else:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 10) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'labels', 'img_size', 'hm loss', 'loss'))
        if rank in [-1, 0]:
            pbar = tqdm(pbar, total=nb)  # progress bar
        optimizer.zero_grad()
        for i, (imgs, hms, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 8) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imgs.shape[-1], hm_loss, loss)

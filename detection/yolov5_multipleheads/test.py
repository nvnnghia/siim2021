...
def test(data,
    ...
    for batch_i, (img, _, targets, paths, shapes, target1) in enumerate(tqdm(dataloader, desc=s)):
        ...
        origin_labels.append(target1.detach().cpu().numpy())

        with torch.no_grad():
            # Run model
            t = time_synchronized()
            (out, train_out), _, logits1 = model(img, augment=augment)  # inference and training outputs
            ...
            prediction = torch.sigmoid(logits1) 

...
augmentation = albumentations.load(f'/home/pintel/nvnn/code/ml/siim/configs/aug/s_0220/0220_hf_cut_sm_0.75_384_no_dis.yaml', data_format='yaml')

with open('/home/pintel/nvnn/dataset/vds_vid/val.txt') as f:
    lines = f.readlines()

for cc, line in tqdm(enumerate(lines[:1])):
...
for cc in range(10):
    ...
    image1 = image.copy()
    seg_out1 = seg_out.copy()

    augmented = augmentation(image=image1, mask=seg_out1)
    image1 = augmented["image"]
    seg_out1 = augmented["mask"]

    seg_out1 = seg_out1*255

    image1[:,:,1] = seg_out1[:,:,0]
    cv2.imwrite(f'hm{cc}.jpg', image1)
...
class DatasetRetriever(Dataset):
    ...
    #     image = cv2.imread(f'{config.TRAIN_ROOT_PATH}/{image_id}.jpg', cv2.IMREAD_COLOR)
    ...
    def load_image_and_boxes(self, index):
        ...
        boxes = getBoxes_yolo(label_path, im_w=512, im_h=512)
    ...
    def validation(self, val_loader):
        ...
        for step, (images, targets, image_ids) in enumerate(val_loader):
            ...
            with torch.no_grad():
                # inputs = []
                # for ii in images:
                #     if isinstance(ii, (np.ndarray, np.generic) ):
...
def filter_image_with_boxes(path_list):
    out_list = []
    for path in path_list:
        with open(path) as f:
            lines = f.readlines()
        if len(lines)>0:
            out_list.append(path.split('/')[-1].split('.')[0])
    return out_list
...
def run_training():
    ...
    train_list_path, train_label_path = get_data(datatxt=f'../data/train_f{opt.fold_number}_s42_cls1.txt')
    valid_list_path, valid_label_path = get_data(datatxt=f'../data/val_f{opt.fold_number}_s42_cls1.txt')

    train_id = filter_image_with_boxes(train_label_path)
    train_list_path = [x for x in train_list_path if x.split('/')[-1].split('.')[0] in train_id]
    train_label_path = [x for x in train_label_path if x.split('/')[-1].split('.')[0] in train_id]

    val_id = filter_image_with_boxes(valid_label_path)
    valid_list_path = [x for x in valid_list_path if x.split('/')[-1].split('.')[0] in val_id]
    valid_label_path = [x for x in valid_label_path if x.split('/')[-1].split('.')[0] in val_id]
...
def get_net():
    ...
    print('Transferred %g/%g items from %s' % (len(state_dict), len(net.state_dict()), weight_file))  # report

...
import yaml
import torch
from models.flexible.modules.common import Conv
from models.flexible.backbone import build_backbone
from models.flexible.neck import build_neck
from models.flexible.head import build_head
# from models.flexible.head import DecoderHead
...
class FlexibleModel(nn.Module):
    ...
    def forward(self, x, augment=False):
        out6 = self.backbone(x)
        out = out6[-3:]
        out = self.fpn(out)
        out = self.pan(out)
        y = self.detection(list(out))
        # seg_out = self.seghead(list(out6))
        return y, out6
...
if __name__ == '__main__':
    ...
    model = models(model_config='../../configs/model_resnet.yaml').to(device)
    # model.fuse()
    import time

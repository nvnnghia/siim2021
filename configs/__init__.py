...
class Experiment(Element):
    def __init__(self, dict):
        ...
        self.anno_only = dict.get('anno_only', False)
        self.keep_valid = dict.get('keep_valid', False)
        self.unified_tfms = dict.get('unified_tfms', False)
        self.mask_version = dict.get('mask_version', 1)
...
class Data(Element):
    def __init__(self, dict):
        ...
        self.image_prefix = dict.get('image_prefix', '')
        self.mask_prefix = dict.get('mask_prefix', '')
...
class Model(Element):
    def __init__(self, dict):
        ...
        if not self.from_checkpoint[0] == '/' and not self.from_checkpoint == 'none':
            # relative path
            self.from_checkpoint = Path(os.path.dirname(os.path.realpath(__file__)) + '/../../') / self.from_checkpoint
            print(self.from_checkpoint)
        self.out_feature = dict.get('out_feature', 1)
        self.teacher_path = dict.get('teacher_path', '')
